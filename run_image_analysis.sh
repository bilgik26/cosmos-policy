...
# Use NVIDIA EGL ICD instead of the default Mesa EGL.
# The container's EGL loader (libEGL.so) only has 50_mesa.json by default.
# We override with an ICD pointing to libEGL_nvidia.so.0 (injected by --nv).
# This allows headless GPU rendering without requiring /dev/dri access.
...
python -m cosmos_policy.experiments.robot.robocasa.analysis.image_analysis \
