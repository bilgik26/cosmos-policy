#!/bin/bash
# 拡張ステップ解析 — PR崖の中間σ挿入 (§3.3) + J-b attn/residual比 (§3.4)
...
python -m cosmos_policy.experiments.robot.robocasa.analysis.extended_step_effrank_jb \
    ...
    --num_denoising_steps_action "${NUM_STEPS:-9}" \
    --num_denoising_steps_future_state 1 --num_denoising_steps_value 1 \
    --data_collection False \
    --num_analysis_episodes "${NUM_EPISODES:-20}" \
    --output_dir cosmos_policy/experiments/robot/robocasa/analysis/results/extended_step_effrank_jb_seedfix
