...
"""
run_robocasa_eval_new.py

Evaluation script for Cosmos Policy on new robocasa (bilgik26/robocasa v1.0.1).
"""
...
# Max steps for each RoboCasa task (from horizons in ATOMIC_TASK_DATASETS)
TASK_MAX_STEPS = {
    "PickPlaceCounterToCabinet": 750,
    "PickPlaceCabinetToCounter": 450,
    "PickPlaceCounterToSink": 600,
    "PickPlaceSinkToCounter": 900,
    "PickPlaceCounterToMicrowave": 1050,
    "PickPlaceMicrowaveToCounter": 750,
    "PickPlaceCounterToStove": 600,
    "PickPlaceStoveToCounter": 450,
    "OpenCabinet": 1050,
    "CloseCabinet": 750,
    "OpenMicrowave": 600,
    "CloseMicrowave": 450,
...
class PolicyEvalConfig:
    ...
    #################################################################################################################
    task_name: str = "TurnOffMicrowave"                                  # Task name (must be a valid new robocasa task name)
    ...
    obj_instance_split: str = "target"                                   # Object instance split: "pretrain" (training objects) or "target" (held-out test objects)
...
def validate_config(cfg: PolicyEvalConfig) -> None:
    ...
    if cfg.task_name not in all_tasks:
        raise ValueError(
            f"Task name '{cfg.task_name}' not found in RoboCasa suite. "
            f"Available atomic tasks: {list(ATOMIC_TASK_DATASETS.keys())}"
        )
...
def create_robocasa_env(cfg: PolicyEvalConfig, seed=None, episode_idx=None):
    """Create a RoboCasa environment using the new robocasa API."""
                ...
                for query_idx in range(cfg.num_queries_best_of_n):
                    ...
                    if cfg.search_depth > 1:
                        ...
                        for depth in range(2, cfg.search_depth + 1):
                            observation_for_depth = prepare_observation(obs, cfg.flip_images)
                            future_state_return_dict_for_depth = get_future_state_prediction(
                                cfg,
                                model=planning_model if planning_model is not None else model,
                                data_batch=action_return_dict["data_batch"],
                                generated_latent_with_action=action_return_dict["generated_latent"],
                            ...
                            future_image_predictions_by_depth.append(
                                future_state_return_dict_for_depth["future_image_predictions"]
                            )

                            future_proprio = future_state_return_dict_for_depth.get("future_proprio")
                            future_observation = prepare_observation(obs, cfg.flip_images)
                            if future_proprio is not None:
                                future_observation["proprio"] = future_proprio

                            future_action_return_dict = get_action(
                                cfg,
                                model,
                                dataset_stats,
                                future_observation,
                                task_description,
                                seed=cfg.seed + query_idx + depth,
                                randomize_seed=cfg.randomize_seed,
                                num_denoising_steps_action=cfg.num_denoising_steps_action,
                                generate_future_state_and_value_in_parallel=False,
                            )
                            actions_by_depth.append(future_action_return_dict["actions"])

                            value_return_dict_for_depth = get_value_prediction(
                                cfg,
                                model=planning_model if planning_model is not None else model,
                                data_batch=future_action_return_dict["data_batch"],
                                future_state_samples_list=future_state_return_dict_for_depth[
                            ...
                            value_predictions_by_depth.append(value_return_dict_for_depth["value_prediction"])

                        if cfg.search_depth_value_aggregation_scheme == "use_last_value":
                            aggregated_value = value_predictions_by_depth[-1]
                        elif cfg.search_depth_value_aggregation_scheme == "average":
                            aggregated_value = np.mean(value_predictions_by_depth)
                        else:
                            raise ValueError(
                                f"Unknown search_depth_value_aggregation_scheme: {cfg.search_depth_value_aggregation_scheme}"
                            )
                        return_dict["value_prediction"] = aggregated_value
                        return_dict["actions"] = actions_by_depth[0]
                        return_dict["future_image_predictions"] = future_image_predictions_by_depth[0]
...
def run_task(
    ...
    for episode_idx in range(cfg.num_trials_per_task):
        ...
        env.reset()
        task_description_raw = env.get_ep_meta()["lang"]
        # Normalize to match pre-computed T5 embeddings cache keys (lowercase, no trailing period)
        task_description = task_description_raw.lower().rstrip(".")
    ...
    log_message(f"  Successes: {sum(successes)}/{len(successes)}", log_file)
    if cfg.use_wandb:
        wandb.log(
            {
