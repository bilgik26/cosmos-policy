"""
Cosmos Policy テーマ2解析スクリプト
mechanism_eval.md テーマ2 (スキル表現の解析) に対応

【テーマ2】アクションの時間方向に抽象化したスキル表現の検証
...
実行方法:
  python -m cosmos_policy.experiments.robot.robocasa.theme2_analysis \\
...
# ── Policy call with Theme2 Capture ─────────────────────────────────────────

def get_action_with_theme2(
    cfg,
    model,
    dataset_stats,
    observation,
    task_description,
    capture: Theme2Capture,
    seed: int,
    num_denoising_steps: int,
):
    """Theme2 フックを有効にしてアクションを生成"""
...
def plot_pca_per_layer(
    ...
    """
    各層の特徴量を PCA (+ 任意で t-SNE) で 2D 可視化。
    ...
    path = out_dir / "theme2_pca_per_layer.png"
...
def plot_tsne_per_layer(
    ...
    fig.suptitle(
        f"Theme 2-1: t-SNE of Block Output Features (k={denoise_step}, action token)\n"
        f"Task: {task_name}  Success: {success_rate:.1%}  "
        f"Color = call index within episode",
        fontsize=11,
    )
    ...
    path = out_dir / "theme2_tsne_per_layer.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    log_message(f"Saved: {path}")
...
def save_features_npz(call_records: List[Dict], probe_layers: List[int], out_dir: Path):
    """
    特徴量を npz として保存（再解析用）。
    ...
    path = out_dir / "theme2_features.npz"
...
@dataclass
class Theme2Config(PolicyEvalConfig):
...
def main():
    import draccus
    cfg: Theme2Config = draccus.parse(Theme2Config)
    ...
    log_message("=== Theme 2 Analysis: Skill Representation in DiT Layers ===")
    ...
    # Register hooks
    capture = Theme2Capture(actual_probe)
    ...
    plot_pca_per_layer(call_records_all, actual_probe, out_dir, cfg.task_name, success_rate, denoise_step=4)
    plot_pca_per_layer(call_records_all, actual_probe, out_dir, cfg.task_name, success_rate, denoise_step=0)

    # t-SNE (try, may be slow)
    plot_tsne_per_layer(call_records_all, actual_probe, out_dir, cfg.task_name, success_rate, denoise_step=4)
    ...
    cka_stats = plot_cka_matrix(
        call_records_all, actual_probe, out_dir, cfg.task_name, success_rate, denoise_step=4
    )
    ...
    log_message("Theme 2 analysis complete!")
    return stats
