...
【目的】
  ...
  このスクリプトでは、テーマ1・テーマ2の検証をアクション生成ではなく
  「画像生成の側」に適用し、次の2つの問いに答える:

  [テーマ1相当] デノイジング過程で future image latent はどのように変化するか?
    1-A: 連続ステップ間の latent 変化量 (||Δx̂₀_img||₂) vs アクション
    1-B: 周波数解析 (2D-FFT) — 低周波成分が先に安定するか?
    1-C: x̂₀_img のノルム推移 (layer ごと、step ごと)

  [テーマ2相当] DiT 内部表現で image token はどのような情報を保持するか?
    2-1: image token 特徴量の PCA 可視化 (タスク進行との対応)
    2-2: action token vs image token の Linear CKA (層別)
    2-3: 線形プロービング — image token 特徴量でスキルフェーズを予測できるか?
...
実行例:
  python -m cosmos_policy.experiments.robot.robocasa.theme_image_analysis \\
...
# ── Plots — Theme 1 (Latent denoising analysis) ───────────────────────────────
def plot_theme1_latent(records: List[Dict], out_dir: Path, task_name: str):
    """
    テーマ1相当: future image latent のデノイジング過程解析

    1-A: 連続ステップ間変化量 (Δ norm) — future_primary vs action
    1-B: 各ステップでのラテントノルム推移 (T 位置別)
    1-C: FFT 周波数解析 — k=0 vs k=4 の周波数成分比較
    """
    N = len(records)
    if N == 0:
        return
    ...
    fig.suptitle(
        f"Theme 1-A (Image): Latent Change ||Δx̂₀||₂ per Denoising Step\n"
    ...
    plt.tight_layout()
    p = out_dir / "image_theme1a_delta_norm.png"
    ...
    fig.suptitle(
        f"Theme 1-B (Image): x̂₀ Latent Norm per Denoising Step\n"
        f"Task: {task_name}  N={N} policy calls",
        fontsize=10,
    )
    ...
    plt.tight_layout()
    p = out_dir / "image_theme1b_latent_norm.png"
    plt.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    log_message(f"Saved: {p}")

    # ── Plot 1-C: FFT 周波数解析 (k=0 vs k=4) ────────────────────────────────
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle(
        f"Theme 1-C (Image): Radial FFT Spectrum of x̂₀ (k=0 vs k=4)\n"
        f"Task: {task_name}  N={N} policy calls",
        fontsize=10,
    )
    ...
    focus_names = {
        ACTION_T_IDX: ("steelblue", "steelblue", "action (T=5)"),
        FUTURE_PRIMARY_T_IDX: ("tomato", "salmon", "future_primary (T=8)"),
        FUTURE_WRIST_T_IDX: ("goldenrod", "khaki", "future_wrist (T=7)"),
    }

    for ax, k_focus in zip(axes, [0, 4]):
        for t_idx in focus_t_idxs:
            ...
            s = sp_arr.std(axis=0)
            color, light, label = focus_names[t_idx]
            r = np.arange(len(m))
        ...
        ax.set_title(f"FFT spectrum at k={k_focus} (σ≈{SIGMA_SCHEDULE[k_focus]:.0f})")
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3, which="both")

    plt.tight_layout()
    p = out_dir / "image_theme1c_fft_spectrum.png"
    plt.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    log_message(f"Saved: {p}")
    ...
    with open(out_dir / "image_theme1_stats.json", "w") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)
    log_message(f"Saved: {out_dir / 'image_theme1_stats.json'}")

    return summary


# ── Plots — Theme 2 (Hidden state analysis) ───────────────────────────────────
def plot_theme2_hidden(records: List[Dict], out_dir: Path, task_name: str):
    """
    テーマ2相当: image token hidden state の層別解析

    2-1: future_primary image token の PCA (7 層 × k=4)
    2-2: action token vs future_primary image token の Linear CKA (層 × ステップ)
    2-3: 線形プロービング (Ridge) — image token でスキルフェーズを予測
    """
    N = len(records)
    if N == 0:
        return
    ...
    plt.tight_layout()
    p = out_dir / f"image_theme2_pca_k{k_focus}.png"
    plt.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    log_message(f"Saved: {p}")
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle(
        f"Theme 2-2 (Image): Linear CKA (action ↔ image token)\n"
    ...
    plt.tight_layout()
    p = out_dir / "image_theme2_cka.png"
    plt.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    log_message(f"Saved: {p}")
    ...
    fig.suptitle(
        f"Theme 2-3 (Image): Linear Probing Accuracy (skill phase, 3-class)\n"
    ...
    plt.tight_layout()
    p = out_dir / "image_theme2_linear_probe.png"
    plt.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    log_message(f"Saved: {p}")
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle(
        f"Theme 2-4 (Image): Hidden State Change per Denoising Step (||Δfeat||₂)\n"
    ...
    plt.tight_layout()
    p = out_dir / "image_theme2_hidden_delta.png"
    plt.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    log_message(f"Saved: {p}")
    ...
    with open(out_dir / "image_theme2_probe_stats.json", "w") as f:
        json.dump(probe_summary, f, indent=2, ensure_ascii=False)
    log_message(f"Saved: {out_dir / 'image_theme2_probe_stats.json'}")
    ...
    with open(out_dir / "image_theme2_cka_stats.json", "w") as f:
        json.dump(cka_summary, f, indent=2, ensure_ascii=False)
    log_message(f"Saved: {out_dir / 'image_theme2_cka_stats.json'}")
...
def main():
    ...
    # Load onto GPU 1 (worker_id=1) — GPU 0 is full (qwen3-vllm uses ~21 GiB)
    ...
    # Run analyses
    log_message("\n=== Running Theme 1 (Latent) Analysis ===")
    plot_theme1_latent(latent_cap.call_records, out_dir, cfg.task_name)

    log_message("\n=== Running Theme 2 (Hidden State) Analysis ===")
    plot_theme2_hidden(token_cap.call_records, out_dir, cfg.task_name)
    ...
    # Save raw data
    log_message("Saving raw latent data...")
    np.savez_compressed(
        out_dir / "image_latent_records.npz",
        ep_idxs=np.array(all_ep_idxs),
        call_idxs=np.array(all_call_idxs),
    )
