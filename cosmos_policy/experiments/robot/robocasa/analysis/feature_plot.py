"""
特徴量の解析・可視化スクリプト (sklearn 不要版)
feature_features.npz を読み込んで numpy のみで PCA・CKA・変化量分析を行う

実行:
  python cosmos_policy/experiments/robot/robocasa/feature_plot.py \
      --npz_path cosmos_policy/experiments/robot/robocasa/analysis/results/action_features/feature_features.npz \
...
def plot_pca_per_layer(feats, episode_labels, call_idx_labels, probe_layers,
    ...
    fig.suptitle(
        f"Feature Analysis-1: PCA of Block Output Features (k={denoise_step}, action token)\n"
    ...
    path = out_dir / f"feature_pca_k{denoise_step}.png"
...
def plot_feature_change_by_layer(feats, probe_layers, out_dir, task_name, success_rate):
    ...
    ax.set_title(
        f"Feature Analysis-2: Feature Change (k=0→k=4) by Layer Depth\n"
        f"Task: {task_name}  Success: {success_rate:.1%}",
        fontsize=11,
    )
    ...
    plt.tight_layout()
    path = out_dir / "feature_feature_change_by_layer.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")
...
def plot_cka_matrix(feats, probe_layers, out_dir, task_name, success_rate, denoise_step=4):
    ...
    ax.set_title(
        f"Feature Analysis-3: Inter-Layer Linear CKA (k={denoise_step}, action token)\n"
        f"Task: {task_name}  Success: {success_rate:.1%}",
        fontsize=11,
    )
    plt.tight_layout()
    path = out_dir / f"feature_cka_matrix_k{denoise_step}.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")
...
def plot_step_change_per_layer(feats, probe_layers, out_dir, task_name, success_rate):
    ...
    fig.suptitle(
        f"Feature Analysis-2 Extension: Feature Change per Denoising Step by Layer Depth\n"
        f"Task: {task_name}  Success: {success_rate:.1%}",
        fontsize=11,
    )
    ...
    plt.tight_layout()
    path = out_dir / "feature_step_change_per_layer.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")
...
def plot_pc1_time_series(feats, episode_labels, call_idx_labels, probe_layers,
    ...
    fig.suptitle(
        f"Feature Analysis-1: PC1 Time Series by Layer (k={denoise_step})\n"
    ...
    plt.tight_layout()
    path = out_dir / f"feature_pc1_timeseries_k{denoise_step}.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")


# ── Plot 5a: t-SNE per layer ─────────────────────────────────────────────

def plot_tsne_per_layer(feats, episode_labels, call_idx_labels, probe_layers,
                        out_dir, task_name, success_rate, denoise_step=4):
    """t-SNE 可視化 (sklearn 必須)。各デノイジングステップで呼び出す。"""
    try:
    ...
    n_episodes = int(episode_labels.max()) + 1
    max_call_idx = max(int(call_idx_labels.max()), 1)
    ...
    for ax, layer_idx in zip(axes, probe_layers):
        ...
        n_comp = min(50, f.shape[1], f.shape[0] - 1)
        pca = skPCA(n_components=n_comp)
        reduced = pca.fit_transform(f)

        perplexity = min(30, f.shape[0] // 3)
    ...
    path = out_dir / f"feature_tsne_per_layer_k{denoise_step}.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")
...
def plot_feature_variance_by_layer(feats, probe_layers, out_dir, task_name, success_rate):
    ...
    fig.suptitle(
        f"Feature Analysis-1 Extension: Feature Variance by Layer\n"
        f"Task: {task_name}  Success: {success_rate:.1%}",
        fontsize=11,
    )
    ...
    plt.tight_layout()
    path = out_dir / "feature_feature_variance.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")
    return results
...
def main():
    ...
    print("Feature Analysis analysis complete!")
