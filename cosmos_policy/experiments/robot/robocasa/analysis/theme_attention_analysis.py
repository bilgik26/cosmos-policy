"""
Cosmos Policy 注意機構（Attention）解析スクリプト
...
実行例（Singularity コンテナ内）:
  python -m cosmos_policy.experiments.robot.robocasa.theme_attention_analysis \\
...
def plot_spatial_heatmaps(spatial_maps: Dict, output_dir: Path):
    """
    output token type ごとに、入力画像 (T=2,3,4) の
    空間注意ヒートマップを保存する。

    選択する (t_out, t_in, k_step) の組み合わせ:
      - t_out ∈ {5 (action), 8 (future_primary), 10 (value)}
      - t_in ∈ {2 (wrist), 3 (primary), 4 (secondary)}
      - k_step ∈ {0, 2, 4}  (初期・中期・終期)
      - block ∈ PROBE_BLOCKS の代表 {0, 13, 27}
    """
    t_outs_plot = [5, 8, 10]
    t_ins_plot = [2, 3, 4]
    steps_plot = [0, 2, 4]
...
def plot_cross_output_comparison(t_matrices: Dict, output_dir: Path):
    """
    複数の出力トークン種別が特定の入力 (T=3: curr_primary) に
    向ける注意の強さを、ブロック深度の関数として比較。
    """
    block_list = sorted(set(k[0] for k in t_matrices))
...
def save_json_stats(t_matrices: Dict, output_dir: Path):
    """数値結果を JSON に保存"""
    block_list = sorted(set(k[0] for k in t_matrices))
    step_list = sorted(set(k[1] for k in t_matrices))
...
def main():
    ...
    parser.add_argument("--num_attn_episodes", type=int, default=3)
    ...
    # Save JSON stats
    stats = save_json_stats(t_matrices, output_dir)
