"""
Cosmos Policy テーマ1（層別）解析スクリプト
theme2_features.npz に保存された各 DiT 層の中間特徴量を使い、
テーマ1 (デノイジング過程) の解析を複数層で実施する。

【テーマ1の再検証 — 層レベル】
  1-A (層別): 連続ステップ間の特徴量変化量 ||feat(k) - feat(k-1)||₂
              「単調増加」パターンが全層で成立するか？浅い層と深い層で異なるか？
  1-B (層別): 特徴空間の固有値スペクトル解析
              各ステップで特徴空間の「実効次元数（有効ランク）」がどう変化するか
              ＝デノイジングで表現が収縮（特化）するか拡張（多様）するか
  1-C (層別): 特徴ノルム ||feat(k)||₂ の層別・ステップ別推移
              深い層ほどノルムが変化するか（x̂₀レベルのスコアノルムは≈371.5で安定していた）
...
実行:
  python -m cosmos_policy.experiments.robot.robocasa.analysis.theme1_layer_analysis \\
...
# pca_numpy と effective_rank は analysis_shared からインポート済み
...
def feature_stats(F: np.ndarray):
    ...
    # Cosine similarity of each sample to the group mean
...
def plot_theme1a_layer(deltas: Dict, out_dir: Path, task_name: str, success_rate: float):
    """
    Theme 1-A (層別): 2 種のプロット
    ...
    fig.suptitle(
        f"Theme 1-A (Layer-wise): Consecutive Feature Change ||feat(k) - feat(k-1)||₂\n"
    ...
    path = out_dir / "theme1a_layer_barchart.png"
    ...
    fig.suptitle(
        f"Theme 1-A (Layer-wise): Feature Change Pattern by Layer\n"
        f"Task: {task_name}  Success: {success_rate:.1%}",
        fontsize=11,
    )
    ...
    axes[1].set_title("Normalized Pattern (same base=k=0→1)", fontsize=10)
    axes[1].axhline(1.0, color="gray", linestyle="--", linewidth=0.8)

    path = out_dir / "theme1a_layer_lineplot.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")

    # ── (c) 全遷移を横軸・全層を色軸にした Heatmap ─────────────────
    ...
    # Normalize per-layer (row) for pattern comparison
    row_max = heatmap.max(axis=1, keepdims=True) + 1e-12
    heatmap_norm = heatmap / row_max

    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    fig.suptitle(
        f"Theme 1-A: Feature Change Heatmap (Layer × Transition)\n"
        f"Task: {task_name}  Success: {success_rate:.1%}",
        fontsize=11,
    )
    ...
    plt.tight_layout()
    path = out_dir / "theme1a_layer_heatmap.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")
...
def plot_theme1b_layer(spec: Dict, out_dir: Path, task_name: str, success_rate: float):
    """
    Theme 1-B (層別):
    (a) 有効ランク (Participation Ratio) の推移 — 各層 × 各ステップ
    (b) 上位 10 固有値の推移 — 選択層での比較
    (c) 分散説明率の累積曲線 — k=0 vs k=4 で各層を比較
    """
    n_layers = len(PROBE_LAYERS)
    layer_colors = plt.cm.cool(np.linspace(0.1, 0.9, n_layers))
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle(
        f"Theme 1-B (Layer-wise): Feature Space Dimensionality across Denoising Steps\n"
        f"Task: {task_name}  Success: {success_rate:.1%}",
        fontsize=11,
    )
    ...
    plt.tight_layout()
    path = out_dir / "theme1b_layer_effrank.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")

    # ── (b) 上位10固有値の比較 (k=0 vs k=4) ──────────────────────
    n_top = 10
    fig, axes = plt.subplots(1, n_layers, figsize=(4 * n_layers, 4.5), sharey=False)
    if n_layers == 1:
        axes = [axes]
    fig.suptitle(
        f"Theme 1-B: Top-10 Singular Value Spectra (k=0 vs k=4) by Layer\n"
        f"Task: {task_name}  Success: {success_rate:.1%}",
        fontsize=11,
    )

    for ax, l in zip(axes, PROBE_LAYERS):
        if 0 not in spec[l] or 4 not in spec[l]:
            ax.set_title(PROBE_LAYER_SHORT.get(l, f"B-{l}"))
            continue
        S0 = spec[l][0]["singular_values"][:n_top]
        S4 = spec[l][4]["singular_values"][:n_top]
        ax.plot(range(n_top), S0, "o-", color="royalblue", linewidth=1.5,
                markersize=4, label="k=0 (σ=80)")
        ax.plot(range(n_top), S4, "s--", color="tomato", linewidth=1.5,
                markersize=4, label="k=4 (σ=4)")
    ...
    plt.tight_layout()
    path = out_dir / "theme1b_layer_spectra.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")

    # ── (c) 累積分散説明率 k=0 vs k=4 per layer ──────────────────
    fig, axes = plt.subplots(1, n_layers, figsize=(4 * n_layers, 4.5), sharey=True)
    if n_layers == 1:
        axes = [axes]
    fig.suptitle(
        f"Theme 1-B: Cumulative Variance Explained (k=0 vs k=4) by Layer\n"
        f"Task: {task_name}  Success: {success_rate:.1%}",
        fontsize=11,
    )
    ...
    for ax, l in zip(axes, PROBE_LAYERS):
        if 0 not in spec[l] or 4 not in spec[l]:
            ax.set_title(PROBE_LAYER_SHORT.get(l, f"B-{l}"))
            continue
        S0 = spec[l][0]["singular_values"]
        S4 = spec[l][4]["singular_values"]
        var0 = (S0**2) / ((S0**2).sum() + 1e-12)
        var4 = (S4**2) / ((S4**2).sum() + 1e-12)
        cum0 = np.cumsum(var0)
        cum4 = np.cumsum(var4)

        max_show = min(50, len(cum0))
        ax.plot(range(max_show), cum0[:max_show], "-", color="royalblue",
                linewidth=1.5, label="k=0 (σ=80)")
        ax.plot(range(max_show), cum4[:max_show], "--", color="tomato",
                linewidth=1.5, label="k=4 (σ=4)")
    ...
    plt.tight_layout()
    path = out_dir / "theme1b_layer_cumvar.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")
...
def plot_theme1c_layer(feats: Dict, out_dir: Path, task_name: str, success_rate: float):
    """
    Theme 1-C (層別): 特徴ノルム ||feat(k)||₂ と call 間コサイン類似度の推移
    """
    k_values = list(range(NUM_DENOISE_STEPS))
    n_layers = len(PROBE_LAYERS)
    layer_colors = plt.cm.cool(np.linspace(0.1, 0.9, n_layers))
    ...
    fig.suptitle(
        f"Theme 1-C (Layer-wise): Feature Norm & Concentration across Denoising Steps\n"
        f"Task: {task_name}  Success: {success_rate:.1%}",
        fontsize=11,
    )
    ...
    # (a) Mean feature norm per layer per step
    ...
    # (b) Cosine similarity to group mean (higher = more concentrated distribution)
    for li, (l, lc) in enumerate(zip(PROBE_LAYERS, layer_colors)):
        lbl = PROBE_LAYER_SHORT.get(l, f"B-{l}")
    ...
    plt.tight_layout()
    path = out_dir / "theme1c_layer_norm.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")
...
def plot_denoising_trajectory_pca(feats: Dict, out_dir: Path, task_name: str, success_rate: float):
    ...
    fig.suptitle(
        f"Theme 1 Extension: Denoising Trajectory in Feature Space (PCA)\n"
        f"Task: {task_name}  Success: {success_rate:.1%}  "
    ...
    for ax, l in zip(axes, PROBE_LAYERS):
        ...
        # Plot: each step as scatter, connect same-call points across steps
        # For clarity, show only a subsample of calls
        n_show = min(30, n_calls or 30)
        show_calls = np.linspace(0, (n_calls or 1) - 1, n_show, dtype=int)

        # Draw trajectories (faint lines)
    ...
    plt.tight_layout()
    path = out_dir / "theme1_denoising_trajectory_pca.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")


# ── 補足: ステップ間コサイン類似度（特徴量の方向変化） ─────────────────
...
def plot_step_cosine_similarity(feats: Dict, out_dir: Path, task_name: str, success_rate: float):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    fig.suptitle(
        f"Theme 1 Extension: Step-to-Step Cosine Similarity of Features\n"
        f"Task: {task_name}  Success: {success_rate:.1%}\n"
    ...
    plt.tight_layout()
    path = out_dir / "theme1_step_cosine_similarity.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")
...
def plot_summary(deltas: Dict, spec: Dict, norm_stats: Dict, out_dir: Path,
    ...
    fig.suptitle(
        f"Theme 1 (Layer-wise) Summary\nTask: {task_name}  Success: {success_rate:.1%}",
        fontsize=13,
    )

    # (0,0) L2 change per step per layer
    for l, lc in zip(PROBE_LAYERS, layer_colors):
    ...
    axes[0, 0].set_title("1-A: Feature Change ||Δfeat||₂ per Layer")
    ...
    axes[0, 1].set_title("1-B: Effective Rank (Feature Space Dimensionality)")
    ...
    axes[1, 0].set_title("1-C: Feature Norm per Layer")
    ...
    # (1,1) Cosine similarity to mean (concentration)
    for li, (l, lc) in enumerate(zip(PROBE_LAYERS, layer_colors)):
        lbl = PROBE_LAYER_SHORT.get(l, f"B-{l}")
    ...
    plt.tight_layout()
    path = out_dir / "theme1_layer_summary.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved: {path}")
...
def main():
    ...
    print("\nRunning Theme 1-A (layer-wise)...")
    deltas = compute_step_deltas(feats)
    layer_means = plot_theme1a_layer(deltas, out_dir, args.task_name, args.success_rate)

    print("\nRunning Theme 1-B (layer-wise, spectral)...")
    spec = compute_spectral_stats(feats)
    plot_theme1b_layer(spec, out_dir, args.task_name, args.success_rate)

    print("\nRunning Theme 1-C (layer-wise, norm)...")
    norm_stats = plot_theme1c_layer(feats, out_dir, args.task_name, args.success_rate)
    ...
    stats_path = out_dir / "theme1_layer_stats.json"
    with open(stats_path, "w") as f:
        json.dump(stats, f, indent=2)
    print(f"\nStats saved: {stats_path}")
    print("Theme 1 (layer-wise) analysis complete!")
