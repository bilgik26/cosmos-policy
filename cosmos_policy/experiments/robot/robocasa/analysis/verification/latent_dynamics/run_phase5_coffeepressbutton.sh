#!/bin/bash
# Phase 5 (ldv_design_v2.md フェーズ5): single-affordance task re-verification, CoffeePressButton.
# Reuses dynamic_vector_field_steering.py / phase4_evaluation_metrics.py unmodified -- both
# scripts were already written task-generically (--task_name), so no new code is needed here,
# only a new invocation. Runs on GPU0 (CloseDrawer runs concurrently on GPU1).
...
python3 -m cosmos_policy.experiments.robot.robocasa.analysis.verification.latent_dynamics.phase4_evaluation_metrics \
    --steering_json cosmos_policy/experiments/robot/robocasa/analysis/results/latent_dynamics_verification/dynamic_vector_field_steering_CoffeePressButton.json \
    --embedding_dir cosmos_policy/experiments/robot/robocasa/analysis/results/latent_dynamics_verification \
