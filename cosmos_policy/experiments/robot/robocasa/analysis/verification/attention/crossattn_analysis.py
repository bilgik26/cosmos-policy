"""
Cosmos Policy クロスアテンション解析スクリプト
...
実行例:
  python -m cosmos_policy.experiments.robot.robocasa.analysis.verification.attention.crossattn_analysis \\
...
def plot_all(per_call: Dict, ep_arr, ci_arr, token_info: dict, task_desc: str,
    ...
    """全デノイジングステップ (k=0〜4) で呼び出される。全7プローブ層を対象とした解析プロット群を生成する。"""
    ...
    # ── Plot 2: Denoising step progression (全プローブ層) ──────────────────
    # 全7層を2行グリッドで表示 (上4列・下3列)
    n_cols_p2 = min(4, n_layers)
    n_rows_p2 = (n_layers + n_cols_p2 - 1) // n_cols_p2
    step_colors = plt.cm.RdYlBu_r(np.linspace(0.1, 0.9, NUM_DENOISE_STEPS))
    fig, axes_p2 = plt.subplots(n_rows_p2, n_cols_p2,
                                figsize=(5 * n_cols_p2, 4.5 * n_rows_p2), sharey=False)
    axes_p2_flat = np.array(axes_p2).flatten()
    fig.suptitle(
        f"Cross-Attention by Denoising Step (k=0→4) — 全{n_layers}プローブ層\nTask: \"{task_desc[:70]}\"",
        fontsize=9,
    )
    ...
    for j in range(n_layers, len(axes_p2_flat)):
        axes_p2_flat[j].set_visible(False)
    plt.tight_layout()
    p = out_dir / f"crossattn_by_denoise_step_k{k_focus}.png"
    plt.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    log_message(f"Saved: {p}")

    # ── Plot 3: Skill phase comparison (早期/中期/後期) — 全プローブ層 ─────
    phase_names = ["Early(reach)", "Mid(grasp)", "Late(place)"]
    phase_colors = ["royalblue", "goldenrod", "tomato"]
    n_cols_p3 = min(4, n_layers)
    n_rows_p3 = (n_layers + n_cols_p3 - 1) // n_cols_p3
    fig, axes_p3 = plt.subplots(n_rows_p3, n_cols_p3,
                                figsize=(5 * n_cols_p3, 4.5 * n_rows_p3), sharey=False)
    axes_p3_flat = np.array(axes_p3).flatten()
    fig.suptitle(
        f"Cross-Attention by Skill Phase (k={k_focus}) — 全{n_layers}プローブ層\n"
        f"Task: \"{task_desc[:70]}\"\n"
    ...
    for i, l in enumerate(probe_layers):
        ...
        ax.grid(True, alpha=0.3)
    for j in range(n_layers, len(axes_p3_flat)):
        axes_p3_flat[j].set_visible(False)
    plt.tight_layout()
    p = out_dir / f"crossattn_by_skill_phase_k{k_focus}.png"
    plt.savefig(p, dpi=150, bbox_inches="tight")
    plt.close()
    log_message(f"Saved: {p}")
    ...
    # ── Plot 5: Attention variability per token — 全プローブ層 ─────────────
    n_cols_p5 = min(4, n_layers)
    n_rows_p5 = (n_layers + n_cols_p5 - 1) // n_cols_p5
    fig, axes_p5 = plt.subplots(n_rows_p5, n_cols_p5,
                                figsize=(5 * n_cols_p5, 4.5 * n_rows_p5))
    axes_p5_flat = np.array(axes_p5).flatten()
    fig.suptitle(
        f"Attention Variability Across Calls (k={k_focus}) — 全{n_layers}プローブ層\n"
    ...
    for i, l in enumerate(probe_layers):
        ...
        ax.set_title(PROBE_LAYER_SHORT.get(l, f"B{l}"), fontsize=9)
        ax.grid(True, alpha=0.3)
    for j in range(n_layers, len(axes_p5_flat)):
        axes_p5_flat[j].set_visible(False)
    plt.tight_layout()
    ...
    # H_all: entropy over all 512 tokens (including padding) — near log(512)≈6.24
    # H_real: entropy restricted to n_real real tokens, renormalized — max=log(n_real)
    # Lower entropy = more concentrated attention on fewer tokens
    ...
    ent_all_matrix = np.zeros((n_layers, NUM_DENOISE_STEPS))
    ent_real_matrix = np.zeros((n_layers, NUM_DENOISE_STEPS))

    for li, l in enumerate(probe_layers):
        entropies_all, entropies_real = [], []
        for k in range(NUM_DENOISE_STEPS):
            a = mean_attn[l][k]  # (Sk,)
            # H over all tokens
            a_clip = np.clip(a, 1e-12, 1.0)
            h_all = -np.sum(a_clip * np.log(a_clip))
            entropies_all.append(h_all)
            ent_all_matrix[li, k] = h_all
            # H over real tokens only (renormalize within n_real)
            a_real = a[:n_real].copy()
            a_real = a_real / (a_real.sum() + 1e-12)
            a_real_clip = np.clip(a_real, 1e-12, 1.0)
            h_real = -np.sum(a_real_clip * np.log(a_real_clip))
            entropies_real.append(h_real)
            ent_real_matrix[li, k] = h_real
        lc = layer_colors[li]
        axes[0].plot(range(NUM_DENOISE_STEPS), entropies_all, "o-", color=lc, linewidth=2,
                     markersize=5, label=PROBE_LAYER_SHORT.get(l, f"B{l}"))
        axes[1].plot(range(NUM_DENOISE_STEPS), entropies_real, "o-", color=lc, linewidth=2,
                     markersize=5, label=PROBE_LAYER_SHORT.get(l, f"B{l}"))

    max_h_all = float(np.log(Sk))
    max_h_real = float(np.log(n_real))
    axes[0].axhline(max_h_all, color="gray", linestyle=":", linewidth=0.8,
                    label=f"max H={max_h_all:.2f}")
    axes[0].set_xlabel("Denoising step k")
    axes[0].set_ylabel("H = -Σp log p")
    axes[0].set_title(f"Entropy (all {Sk} tokens)\n(↓ = focused; max={max_h_all:.2f})")
    ...
    axes[1].axhline(max_h_real, color="gray", linestyle=":", linewidth=0.8,
                    label=f"max H={max_h_real:.2f}")
    axes[1].set_xlabel("Denoising step k")
    axes[1].set_ylabel("H_real = -Σp log p  (real tokens only)")
    axes[1].set_title(f"Entropy (real {n_real} tokens only)\n(↓ = focused; max={max_h_real:.2f})")
    ...
    # Right: real-token entropy heatmap (layer × step)
    im = axes[2].imshow(ent_real_matrix, aspect="auto", cmap="viridis_r",
                        vmin=0, vmax=max_h_real)
    axes[2].set_xticks(range(NUM_DENOISE_STEPS))
    axes[2].set_xticklabels([f"k={k}" for k in range(NUM_DENOISE_STEPS)])
    axes[2].set_yticks(range(n_layers))
    axes[2].set_yticklabels([PROBE_LAYER_SHORT.get(l, f"B{l}") for l in probe_layers])
    axes[2].set_title(f"Real-Token Entropy Heatmap\n(max={max_h_real:.2f}; ↓ = focused)")
    plt.colorbar(im, ax=axes[2], label="H_real")
    for i in range(n_layers):
        for j in range(NUM_DENOISE_STEPS):
            v = ent_real_matrix[i, j]
            axes[2].text(j, i, f"{v:.2f}", ha="center", va="center",
                         fontsize=7, color="white" if v < max_h_real * 0.6 else "black")
    plt.tight_layout()
...
def main():
    ...
    log_message("=== Cross-Attention Analysis ===")
    ...
    for ep_idx in range(cfg.num_crossattn_episodes):
        ...
        for t in range(max_steps):
            ...
            if action_queue:
                ...
                # reward>0 判定は常にFalseを返すバグだった（seed195で50episode中
                # 0成功という、他スクリプト(55-60%成功)と矛盾する結果で発覚）。
                # feature_analysis.py等と同様にenv._check_success()で判定する。
    ...
    save_dict = {
        ...
        "episode_success_index": ep_success_index,
        "episode_success_flag": ep_success_flag,
        "episode_success_per_call": np.array(
            [bool(episode_success.get(int(e), False)) for e in ep_arr]
        ),
    }
    ...
    npz_path = out_dir / "crossattn.npz"
    ...
    with open(out_dir / "crossattn_meta.json", "w", encoding="utf-8") as f:
        json.dump(meta, f, indent=2, ensure_ascii=False)
    log_message(f"Saved: {out_dir / 'crossattn_meta.json'}")

    # Generate plots
    log_message("\nGenerating plots...")
    # 全デノイジングステップ (k=0〜4) で plot_all を実行
    for k in range(NUM_DENOISE_STEPS):
        plot_all(
            per_call=per_call,
            ep_arr=ep_arr,
