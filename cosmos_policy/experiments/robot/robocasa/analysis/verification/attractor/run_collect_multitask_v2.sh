#!/bin/bash
# attractor_verification 追加検証: per-episode success flag付き再収集 (課題A用)
# collect_multitask.py の episode_success 未保存バグを修正した版で再実行。
# 既存の collect/ (成功フラグなし) は他セクションの成果物として温存し、上書きしない。
...
python -m cosmos_policy.experiments.robot.robocasa.analysis.verification.attractor.collect_multitask \
    ...
    --output_dir cosmos_policy/experiments/robot/robocasa/analysis/results/attractor_verification/collect_v2
