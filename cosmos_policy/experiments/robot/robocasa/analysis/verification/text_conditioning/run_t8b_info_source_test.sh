#!/bin/bash
# T8b: gripperプローブの情報源テスト — review_verification_report_2.md §3
...
python -m cosmos_policy.experiments.robot.robocasa.analysis.verification.text_conditioning.t8b_info_source_test \
    ...
    --num_pool_episodes "${NUM_POOL_EPISODES:-15}" \
    --max_pairs "${MAX_PAIRS:-60}" \
    --label_source "${LABEL_SOURCE:-current_proprio}" \
    --output_dir cosmos_policy/experiments/robot/robocasa/analysis/results/t8b_info_source
